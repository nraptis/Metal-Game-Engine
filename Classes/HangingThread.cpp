...
void HangingThread::Draw3D() {
    ...
    for (int i=0;i<mSegmentList.mCount;i++) {
        ...
        Graphics::SetColor(1.0f, 0.0f, 0.0f, 1.0f);
        Graphics::DrawBox(aX, aY, aZ,
                          aX, aY, aZ + 0.15f, 0.25f);
...
void HangingThread::Generate() {
    ...
    for (int i=0;i<mKnotCount;i++) {
        ...
        mSpline.Add(aCur->mX, aCur->mY, aCur->mZ);
    }
    mSpline.Solve();
    ...
    float aHopSize = 0.5f;
    //if (gRand.Get(3) == 1) { aHopSize = 0.2f; }
    //if (gRand.Get(3) == 1) { aHopSize = 0.125f; }
